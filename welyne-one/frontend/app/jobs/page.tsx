...
export default function JobDetailPage() {
  ...
  return (
    <div>
      ...
      {!hasSpec && !canWrite && (
        <p className="card" style={{ color: "var(--ink-soft)", fontSize: 14 }}>
          Fiche de poste pas encore générée — en attente d&apos;un recruteur.
        </p>
      )}
      ...
      <SourcingPanel jobId={id} token={token} canWrite={canWrite} />
    </div>
  );
}
...
function WeightSlider({ label, value, onChange, onCommit, disabled }: { label: string; value: number; onChange: (v: number) => void; onCommit: (v: number) => void; disabled?: boolean }) {
  return (
